...
pub async fn like_post(pool: web::Data<sqlx::PgPool>, post_id: web::Path<i32>, token: String) -> HttpResponse {
...
pub async fn unlike_post(pool: web::Data<sqlx::PgPool>, post_id: web::Path<i32>, token: String) -> HttpResponse {
    let user = match get_user_from_token(&pool, &token).await {
        Ok(user) => user,
        Err(response) => return response,
    };
